...
] as Omit<User, 'avatarUrl' | 'avatarColor' | 'certified' | 'fairplayPct' | 'activityPct' | 'score' | 'reviewCount' | 'profilePct' | 'isAdmin' | 'isOwner' | 'username'>[])
  .map((u) => ({ ...u, avatarUrl: '', avatarColor: null, certified: false, fairplayPct: null, activityPct: 0, score: null, reviewCount: 0, profilePct: 0, isAdmin: false, isOwner: false, username: u.name.toLowerCase().replace(/[^a-z0-9]+/g, '') }));
