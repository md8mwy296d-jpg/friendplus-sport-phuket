...
import type { AnnouncementInput } from '@/lib/news';
import { useI18n } from '@/lib/i18n';
import { cn } from '@/lib/utils';

const MAX_SOURCE_BYTES = 15 * 1024 * 1024;

/** News composer for admins and certified accounts (pinning is admin-only). */
export default function NewsComposer({ isAdmin, onPost }: { isAdmin: boolean; onPost: (i: AnnouncementInput) => Promise<boolean> }) {
  const { t } = useI18n();
  ...
  const submit = async (e: FormEvent) => {
    ...
    setBusy(true);
    const ok = await onPost({ title, body, image, pinned });
    setBusy(false);
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        ...
        <Megaphone className="h-5 w-5" /> {t('news.composer.heading')}
      </button>
    );
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-3 rounded-[20px] border border-[#EADFC8] bg-white p-4 shadow-[0_2px_8px_rgba(11,46,43,.06)] sm:p-5">
      <p className="flex items-center gap-2 text-sm font-bold text-[#0B2E2B]"><Megaphone className="h-4 w-4 text-[#FF6B4A]" /> {t('news.composer.heading')}</p>
      <input value={title} onChange={(e) => setTitle(e.target.value.slice(0, 120))} placeholder={t('news.composer.titlePh')} className={cn(field, 'font-semibold')} />
      <textarea value={body} onChange={(e) => setBody(e.target.value.slice(0, 2000))} placeholder={t('news.composer.bodyPh')} rows={4} className={cn(field, 'resize-none')} />
      {preview && (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isAdmin && (
          <button type="button" onClick={() => setPinned((v) => !v)} aria-pressed={pinned}
            ...
            <Pin className="h-3.5 w-3.5" /> {t('news.composer.pin')}
          </button>
        )}
        ...
        <button type="submit" disabled={busy || !title.trim()}
          ...
          {busy ? t('news.composer.publishing') : t('news.composer.publish')}
        </button>
      </div>
    </form>
  );
